...
class Chunker:
    ...
    def _split_long_section(self, text: str, soft_cap: int = 150, hard_cap: int = 200, overlap: int = 30) -> List[str]:
        # Split by table row boundary (| |) or sentence boundary (. )
        ...
        for unit in units:
            if not current_chunk:
                ...
                if self._get_tokens(combined) > soft_cap and self._get_tokens(current_chunk) > 0:
                    ...
                    # overlap: start new chunk with the last unit if it's small enough, else just current unit
...
if __name__ == "__main__":
    ...
    for pdir in processed_dirs:
        ...
        logger.info(f"Chunked {os.path.basename(pdir)} -> {len(chunks)} chunks")
