...
class Embedder:
    ...
    def embed(self, chunks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for chunk in chunks:
            ...
            text = chunk.get("text", "")
            # Critical EC-1.8: Prepend scheme name to text for embedding
            full_text = f"{scheme_name}\n\n{text}"
            texts.append(full_text)

        logger.info(f"Embedding {len(texts)} chunks...")
        # BGE models require normalize_embeddings=True for cosine similarity
...
if __name__ == "__main__":
    ...
    logger.info(f"Successfully embedded {len(result['chunks'])} chunks.")
    logger.info("Saved data/index/embeddings.npy, data/index/chunks.jsonl, data/index/embedder.json")
