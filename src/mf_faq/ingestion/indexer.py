...
class Indexer:
    ...
    def build(self) -> None:
        ...
        # Using IndexFlatIP since embeddings from Phase 1.5 were normalized (cosine similarity)
        ...
        # Tokenize by simple whitespace/punctuation for BM25
        tokenized_corpus = [chunk["text"].lower().split() for chunk in chunks]
        bm25 = BM25Okapi(tokenized_corpus)
        with open(os.path.join(self.staging_dir, "bm25.pkl"), "wb") as f:
            pickle.dump(bm25, f)
            
        # 4. Copy chunks.jsonl and embedder.json to staging
        ...
        # Take the first stable hash seen for each scheme
        source_hashes = {}
        for c in chunks:
            sid = c["scheme_id"]
